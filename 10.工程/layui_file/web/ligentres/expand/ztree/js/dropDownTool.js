...
(function ($) {
    ...
    function checkNode(event, treeId, treeNode) {
    	var cyProps = $("#" + treeId).parents('[cytype="dropDownTool"]').attr("cyProps");
    	if (!cyProps) {
            return;
        }
        cyProps = cyProps ? cyProps : "";
    	...
    	var idkey = cyProps.idkey || 'id';
    	var namekey = cyProps.namekey || 'name';
        ...
        if (treeId.indexOf("radio") !== -1) {
            ...
            $("#" + treeId).parents('[cytype="dropDownTool"]').find(".combo-text").val(treeNode[namekey]);
            $("#" + treeId).parents('[cytype="dropDownTool"]').find(".form-input").val(treeNode[idkey]);
        }
        //多选
        else {
            ...
            for (var i = 0; i < nodes.length; i++) {
                ...
                checkIds += nodes[i][idkey] + (i === nodes.length - 1 ? "" : ",");
                checkNames += nodes[i][namekey] + (i === nodes.length - 1 ? "" : ",");
            }
    ...
    function getSetting(cyProps){
    	var idkey = cyProps.idkey || 'id';
    	var pidkey = cyProps.pidkey || 'pid';
    	var namekey = cyProps.namekey || 'name';
    	var _setting = null;
    	if(cyProps.checkbox === "true"){
    		// 复选框
    		_setting = {
        	        view: {
    ...
    function createDropDownDom($inputValue, cyProps) {
        ...
        var ztree = $.fn.zTree.init($("#" + ztreeId), getSetting(cyProps), treeData);
    ...
    // 选中后回调函数
	function callBackTreeCheck(){
	    if(typeof zTreeCheck != 'undefined' && zTreeCheck instanceof Function)
	    	zTreeCheck();
	}
