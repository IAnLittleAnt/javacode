  ...
  ,admin = {
    ...
    ,sendAuthCode: function(options){
      options = $.extend({
        seconds: 60
        ,elemPhone: '#LAY_phone'
        ,elemHasVercode: '#LAY_hasVercode'
        ,elemVercode: '#LAY_vercode'
        ,elemSmsType: '#LAY_smsType'
        ,elemSmscode: '#LAY_smscode'
        ,elemUserId: '#LAY_userId'
      }, options);
      ...
      options.elemHasVercode = $(options.elemHasVercode);
      options.elemVercode = $(options.elemVercode);
      options.elemSmsType = $(options.elemSmsType);
      options.elemSmscode = $(options.elemSmscode);
      options.elemUserId = $(options.elemUserId);

      btn.on('click', function(){
        var elemPhone = options.elemPhone
    		, elemHasVercode = options.elemHasVercode
	    	, elemVercode = options.elemVercode
	    	, elemSmsType = options.elemSmsType
	    	, elemSmscode = options.elemSmscode
        	, elemUserId = options.elemUserId
        	, mobile = elemPhone.val()
        	, hasVercode = elemHasVercode.val()
        	, vercode = elemVercode.val()
        	, smsType = elemSmsType.val()
        	, userId = elemUserId.val(); 
        ...
        // hasVercode == 1时需要校验图片验证码
        if(hasVercode == 1 && (vercode == '' || vercode == null)){
        	elemVercode.focus();
            layer.msg('请输入图片验证码', {
                icon: 5
                ,shade: 0
    ...
    ,tabsBodyChange: function(index, options){
      ...
      /*var isTabsChange = true;
      if(options.text){
    	  isTabsChange = false; 
    	  //判断页面是否在菜单栏中，存在就切换页面标签主体
          var menus = $('#LAY-system-side-menu a');
          menus.each(function(index){
              var a = $(this)
              ,layhref = a.attr('lay-href')
              ,layevent = a.attr('layadmin-event');
              
              if(layhref === url || layevent === url){
              	isTabsChange = true;
              	return;
              }
          });
    ...
    ,serach: function(othis){
      othis.off('keypress').on('keypress',function(e){
        ...
        if(e.keyCode === 13){
          ...
          menus.each(function(index){
              var a = $(this);
              var laytext = a.attr('lay-text');
              if(laytext === serachKey){
            	  serachUrl = a.attr('lay-href');
            	  return false;
              }
          });
          // 完整查找不到，就模糊查找
          if(serachUrl == null){
        	  menus.each(function(index){
                  var a = $(this);
                  var laytext = a.attr('lay-text');
                  if(laytext!=null && laytext.indexOf(serachKey)>=0){
                	  serachUrl = a.attr('lay-href');
                	  serachKey = laytext;
                	  return false;
                  }
          ...
          if(serachUrl == null){
        	  layer.msg('找不到内容为['+serachKey+']的栏目', {
        			icon: 5,
        			time: 6000,
        			offset: '15px'
        		});
              ...
              topLayui.index.openTabsPage(serachUrl, serachKey);
          }
          // 清空输入框
          this.value = '';
        }       
